// ----------------------------------------------------------------------------
// petalosim | TileHamamatsuVUVNoLXe.cc
//
...
TileHamamatsuVUVNoLXe::TileHamamatsuVUVNoLXe() : TileGeometryBase(),
                                                 tile_x_(30.9 * mm),
                                                 tile_y_(30.7 * mm),
                                                 tile_z_(2.3 * mm),
                                                 sipm_pitch_(7.5 * mm),
...
TileHamamatsuVUVNoLXe::~TileHamamatsuVUVNoLXe()
{
}

void TileHamamatsuVUVNoLXe::Construct()
{
  ...
  G4double zpos_quartz = tile_z_/2 - quartz_thick_/2.;
  new G4PVPlacement(0, G4ThreeVector(0., 0., zpos_quartz), quartz_logic,
                    "TILE_QUARTZ_WINDOW", tile_logic, false, 0, false);
  ...
  for (int j = 0; j < n_rows_; j++)
  {
    for (int i = 0; i < n_columns_; i++)
    {
      ...
      G4double z_pos = tile_z_ / 2. - quartz_thick_ - sipm_dim.z() / 2.;
